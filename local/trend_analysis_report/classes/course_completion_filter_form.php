...
$PAGE->requires->js(new moodle_url($CFG->wwwroot.'/local/trend_analysis_report/js/course_completion.js'));


class course_completion_filter_form extends moodleform {
    ...
    public function definition() {
        ...
        $course_arr = course_dropdown_format_list(get_courses());
        ...
        $mform->addElement('html', html_writer::select(createDropdown($course_arr), 'course_name', '', array(),array('class'=>'form-control')));
        $mform->addElement('html', html_writer:: end_tag('div'));
        ...
        $mform->addElement('html', html_writer::select(createDropdown(get_system_user_list()), 'user', '', array(),array('class'=>'form-control')));
        $mform->addElement('html', html_writer:: end_tag('div'));

        $mform->addElement('html', html_writer:: start_tag('div',array('class'=>'form-group col-lg-3 form-group-ele')));
        $mform->addElement('html', html_writer:: tag('label','Manager',array('for'=>'manager','class'=>'col-md-12')));
        $mform->addElement('html', html_writer::select(createDropdown(get_all_manager_list()), 'manager', '', array(),array('class'=>'form-control')));
        $mform->addElement('html', html_writer:: end_tag('div'));

        $mform->addElement('html', html_writer:: start_tag('div',array('class'=>'form-group col-lg-3 form-group-ele')));
        $mform->addElement('html', html_writer:: tag('label','User Status',array('for'=>'user_status','class'=>'col-md-12')));
        $mform->addElement('html', html_writer::select(createDropdown(user_status()), 'user_status', '', array(),array('class'=>'form-control')));
        $mform->addElement('html', html_writer:: end_tag('div'));
        ...
        $mform->addElement('html', html_writer::tag('a', 'Reload', array('href'=>$CFG->wwwroot.'/local/trend_analysis_report/course_completion.php?m=1_2','class'=>'btn btn-secondary')));
        $mform->addElement('html', html_writer:: end_tag('div'));

...
if ($hassiteconfig OR has_capability('local/trend_analysis_report:view', $PAGE->context,$USER->id, false)) {
    ...
    $ADMIN->add($section, new admin_externalpage('local_trend_analysis_report',
            get_string('pluginname', 'local_trend_analysis_report'),
            new moodle_url('/local/trend_analysis_report/')
    ));
}

...
function send_accident_report_email($from, $subject, $message, $attachment_path, $file_name,$report_title) {
...
function get_accident_report_list()
{
    ...
    $sql1   = "SELECT * FROM mdl_high_accident_report_send_notification WHERE month= MONTH(CURRENT_TIMESTAMP) AND year= YEAR(CURRENT_TIMESTAMP)"; // BM changed for MOOD-241
    ...
    if($result2->act_of_physical_violence_status==1){
        ...
        if ($acc_received[74] >= $act_of_physical_violence ){
            ...
            if(!isset($notificationArr[74])) {
                ...
                $DB->insert_record('high_accident_report_send_notification', $dt);
